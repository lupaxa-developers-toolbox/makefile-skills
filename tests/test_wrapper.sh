...
# init clones from local repo path (skills/ + templates/ — sparse checkout)
# Lifecycle output is quiet: only ==> status lines (no git chatter).
out="$(NO_COLOR=1 make -C "$CONSUMER" init MAKEFILES_REPO="$BARE" 2>&1)"
assert_contains "$out" "==> Cloning skills"
assert_contains "$out" "==> makefile skills at"
assert_contains "$out" "Creating makefiles.config"
assert_not_contains "$out" "Already on"
assert_not_contains "$out" "From "
assert_not_contains "$out" "Already up to date"
assert_not_contains "$out" "Fetching "
assert_not_contains "$out" "remote:"
assert_not_contains "$out" "Receiving objects"
assert_not_contains "$out" "Enumerating objects"
...
# Top-level of .makefiles must be skills + templates only (.git is hidden / excluded)
top_level="$(find "$CONSUMER/.makefiles" -mindepth 1 -maxdepth 1 ! -name .git | sort)"
printf '%s\n' "$top_level" | grep -q '/skills$'
printf '%s\n' "$top_level" | grep -q '/templates$'
test ! -e "$CONSUMER/.makefiles/mkdocs"
test ! -e "$CONSUMER/.makefiles/examples"
test ! -e "$CONSUMER/.makefiles/tests"
...
out="$(NO_COLOR=1 make -C "$CONSUMER" update MAKEFILES_REF=head 2>&1)"
assert_contains "$out" "==> Updating makefile skills"
assert_contains "$out" "==> makefile skills at"
assert_not_contains "$out" "Already on"
assert_not_contains "$out" "From "
assert_not_contains "$out" "Already up to date"
