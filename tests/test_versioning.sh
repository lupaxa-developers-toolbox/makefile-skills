...
# Preflight: last stable tag must match current_version. Draft tags do not count.
set_version "1.2.3"
run_bump bump-patch
assert_version "1.2.4"

set_version "1.2.3"
git -C "$CONSUMER" tag v1.2.3
run_bump bump-patch
assert_version "1.2.4"

set_version "1.2.3"
git -C "$CONSUMER" tag v1.2.5
err="$(make -C "$CONSUMER" bump-minor 2>&1 >/dev/null)" || rc=$?
if [ "${rc:-0}" -ne 2 ]; then
  echo "ASSERT: expected make bump-minor to exit 2 when last stable tag is ahead, got ${rc:-0}" >&2
  exit 1
fi
assert_contains "$err" "ERROR"
assert_contains "$err" "1.2.3"
assert_contains "$err" "1.2.5"
assert_contains "$err" "Do not tag by hand"
file_ver="$(sed -n 's/^current_version = "\([^"]*\)"/\1/p' "$CONSUMER/.bumpversion.toml")"
[ "$file_ver" = "1.2.3" ] || {
  echo "ASSERT: mismatch must not change current_version (got $file_ver)" >&2
  exit 1
}
